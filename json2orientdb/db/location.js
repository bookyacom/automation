...
const location = sequelizeDB.define('location', {
  ...
  classMethods: {
    ...
    createBasedLocation: function({ city, country, latitude, longitude, type }) {
      let name = city || country;
      assert(name && country && latitude && longitude);
      ...
      return this.findOrCreate({
        ...
        defaults: {
          ...
          point: Sequelize.fn('ST_GeomFromText', `POINT(${latitude} ${longitude})`)
        }
      }).spread(function(location, created) {
        location = location.toJSON();
        if (!location) throw new Error('location unable to create');
