...
def clean_artist(festival):
    ...
    Exmpl: Lollapalooza Argentina -> Lollapalooza
    """
    cities = ['Block party','Solardo','TS5','RAM','Metropolis','Sasha & John Digweed','Play','Nina Kraviz','Levelz','Knee Deep','Jamie XX','Jackmaster','FLOATING POINTS','Feel my Bicep','Circus','Chemical Brothers','ANNIE MAC PRESENTS','Ants','Unite','Klockgebouw','Katermukke','Fritz Kalkbrenner','GEM','Diyanmic','Afterlife','A State Of Trance', 'Martin Garrix', 'Mosaic', 'Paradise', 'Kompakt', 'Drumcode', 'Early NYE', 'NYD', 'NYE', 'Figure', 'Jooris Voorn & Friends', 'Paul Kalkbrenner', 'Selectors']
...
def clean_month(festival):
    """
    Removes months in a festival

    Arguments:
    festival: name of festival

    Return: 
    festival_: name of festival without month

    Exmpl: Lollapalooza August -> Lollapalooza
    """
    cities = ['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'Oktober', 'November', 'Dezember']
...
def clean_festival(festival):
    ...
    festival_req = festival_req.replace('Weekend', '')
    festival_req = festival_req.replace('Week', '')
    festival_req = festival_req.replace('Day', '')
    festival_req = festival_req.replace('Week', '')
    festival_req = festival_req.replace('ADE', '')
    festival_req = festival_req.replace('years', '')
    festival_req = festival_req.replace('1st', '')
    festival_req = festival_req.replace('2nd', '')
    festival_req = festival_req.replace('3rd', '')
    for num in range(21):
        festival_req = festival_req.replace(str(num), '')

    festival_req = clean_country(festival_req)
    festival_req = clean_city(festival_req)
    festival_req = clean_artist(festival_req)
    festival_req = clean_month(festival_req)
    festival_req = festival_req.strip()
